...
function volumeUp(DJName)
    if xSound:soundExists(DJName) then
        if currentDJ.volume + 0.05 >= 1 then
            currentDJ.volume = 1.0
            xSound:setVolume(DJName, 1.0)
            ShowNotification('~g~Maximum volume reached')
        else
            currentDJ.volume = currentDJ.volume + 0.05
            xSound:setVolume(DJName, currentDJ.volume + 0.05)
            ShowNotification('~g~Volume set to: ' .. (currentDJ.volume + 0.05) * 100 .. '%')
        end
...
function volumeDown(DJName)
    if xSound:soundExists(DJName) then
        ...
        if currentDJ.volume - 0.05 <= 0 then
